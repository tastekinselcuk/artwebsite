    ...
    <main class="max-w-7xl mx-auto px-6 py-8 space-y-8 animate-fade-in">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-card rounded-xl p-6 shadow-sm border border-border">
          ...
          <div class="mt-4 text-xs text-muted-foreground">Mağaza modülü yakında aktif olacak.</div>
        </div>

        <div
          class="bg-card rounded-xl p-6 shadow-sm border border-border flex flex-col justify-center gap-3"
        >
          <div class="flex gap-2">
            <router-link
              to="/admin/ebru"
              class="flex-1 flex items-center justify-center py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition text-xs font-medium"
            >
            ...
            <router-link
              to="/admin/shop"
              class="flex-1 flex items-center justify-center py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition text-xs font-medium"
            >
              <Plus class="w-3 h-3 mr-1" /> Shop
            </router-link>
          </div>
          <router-link
            to="/admin/settings"
            class="flex items-center justify-center w-full py-2 bg-muted text-foreground hover:bg-muted/80 rounded-lg transition text-sm font-medium"
          >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div
          class="lg:col-span-2 bg-card rounded-xl shadow-sm border border-border overflow-hidden"
        >
        ...
        <div
          class="bg-card rounded-xl shadow-sm border border-border p-6 flex flex-col justify-center items-center text-center"
        >
          <div
            class="w-32 h-32 rounded-full border-8 border-muted flex items-center justify-center relative mb-4"
          >
            <div
      ...
      <div class="bg-card rounded-xl shadow-sm border border-border overflow-hidden min-h-[400px]">
        <div
          class="p-4 border-b border-border flex flex-col md:flex-row justify-between md:items-center gap-4 bg-muted/10"
        >
          <h3 class="font-bold text-lg text-foreground flex items-center gap-2">
            ...
            <tbody class="divide-y divide-border">
              ...
              >
                <td class="px-6 py-4 font-medium text-foreground flex items-center gap-3">
                  <div
                    class="w-10 h-10 rounded-md overflow-hidden bg-muted flex-shrink-0 border border-border"
                  >
                    <img
...
const filteredArtworks = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    combined = combined.filter(
      (item) =>
        item.title.tr.toLowerCase().includes(query) || item.title.en.toLowerCase().includes(query),
    );
  }
