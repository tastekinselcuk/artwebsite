<template>
  <div class="min-h-screen py-20 px-6 bg-background">
    <div class="max-w-7xl mx-auto mb-20 text-center space-y-4 animate-fade-in">
      ...
      <div class="w-24 h-1 bg-indigo-500/20 mx-auto rounded-full mt-6"></div>
    </div>
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div
        v-else-if="galleryStore.photographs.length > 0"
      ...
      >
        <button
          ...
          class="group relative aspect-square overflow-hidden rounded-2xl bg-muted shadow-sm hover:shadow-2xl transition-all duration-500 cursor-zoom-in border border-border/50 animate-fade-in-up hover:-translate-y-1"
        >
          <img
            :src="photo.image || 'https://placehold.co/600x400?text=No+Image'"
          ...
          >
            ...
            <div class="text-left text-white drop-shadow-md">
              <p class="text-xs font-mono mb-2 text-white/80 uppercase tracking-widest">{{ photo.year }}</p>
          ...
          >
            ...
            >
              ...
              <div class="relative bg-muted/30 flex items-center justify-center p-4 md:p-8 h-[50vh] md:h-auto">
                <img
                  :src="selectedPhoto.image || 'https://placehold.co/600x400?text=No+Image'"
                ...
                <div class="space-y-6">
                  ...
                  <div class="prose prose-lg text-muted-foreground">
                    <p class="leading-relaxed">
                      {{
                        currentLocale === 'en'
                          ? selectedPhoto.description.en
                          : selectedPhoto.description.tr
                      }}
                    </p>
                  </div>
                </div>
...
import { X, Eye, Maximize2, Camera, Calendar } from 'lucide-vue-next'
...
// Sayfa açılınca Supabase'den foto verilerini çek
onMounted(() => {
...
const openPhoto = async (photo) => {
  ...
  await galleryStore.incrementPhotoViews(photo.id)
}
</script>
