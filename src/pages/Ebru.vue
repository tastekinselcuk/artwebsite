<template>
  <div class="min-h-screen py-20 px-6 bg-background">
    <div class="max-w-7xl mx-auto mb-20 text-center space-y-4 animate-fade-in">
      ...
      <p
        class="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto font-light leading-relaxed"
      >
        {{ t("ebru.subtitle") }}
      </p>
    ...
    <div class="w-full max-w-[1920px] mx-auto px-4">
      ...
      >
        <div
          v-for="n in 12"
          :key="n"
          class="break-inside-avoid mb-4 aspect-[3/4] rounded-2xl bg-muted/20 animate-pulse border border-border"
        ></div>
      </div>

      <div
        v-else-if="galleryStore.error"
        class="text-center py-20 bg-destructive/5 rounded-2xl border border-destructive/10"
      >
      ...
      >
        <button
          ...
          class="group relative w-full break-inside-avoid mb-4 overflow-hidden rounded-2xl bg-muted shadow-sm hover:shadow-2xl transition-all duration-500 cursor-zoom-in border border-border/50 animate-fade-in-up block"
        >
          <img
          ...
          <div
            class="absolute inset-0 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0"
          >
            <div
              class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center text-white border border-white/20 transform scale-0 group-hover:scale-100 transition-transform duration-300 delay-100"
            >
              <Maximize2 class="w-4 h-4" />
            </div>

            <div class="text-left text-white drop-shadow-md">
              <p class="text-[10px] font-mono mb-1 text-white/80 uppercase tracking-widest">
                {{ artwork.year }}
              </p>
              <h3 class="text-lg font-display font-semibold leading-tight">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                class="relative bg-muted/30 flex items-center justify-center p-4 md:p-8 h-[50vh] md:h-auto"
              >
                <img
              ...
              <div class="p-8 md:p-12 flex flex-col justify-center overflow-y-auto bg-card">
                <div class="space-y-6">
                  <div>
                    <span
                      class="inline-block px-3 py-1 bg-primary/10 text-primary text-xs font-bold uppercase tracking-wider rounded-full mb-3"
                    >
                      {{ t("ebru.collectionBadge") }}
                    </span>
                    <h2
                      class="text-3xl md:text-5xl font-display font-bold text-foreground leading-tight"
                    >
                  ...
                  <div
                    class="flex items-center gap-6 text-sm text-muted-foreground border-y border-border py-4"
                  >
                    ...
                    <div class="flex items-center gap-2">
                      <Eye class="w-4 h-4" />
                      <span>{{ selectedArt.views }} {{ t("gallery.views") }}</span>
                    </div>
                  </div>
